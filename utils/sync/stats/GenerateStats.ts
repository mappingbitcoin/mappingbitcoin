...
const STATS_OUTPUT_FILE = path.resolve('public', 'data', 'stats.json');
...
export async function generateStats() {
    ...
    const outputDir = path.dirname(STATS_OUTPUT_FILE);
    await fs.mkdir(outputDir, { recursive: true });

    await fs.writeFile(STATS_OUTPUT_FILE, JSON.stringify(stats, null, 2));
    console.log(`✅ Generated stats: ${stats.totalVenues} venues, ${stats.countries} countries, ${stats.continents} continents`);
}
